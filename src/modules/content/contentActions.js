...
import {
  getFilter, getPageSelector, getSearchValueSelector, getPageSizeSelector,
} from './contentSelectors';
import {
  TRENDING, TOPRATED, COMINGSOON, ADD_GENRES, ADD_DETAILS,
  SET_LOADING, ADD_ITEMS, SET_FILTER, SET_CURRENT_PAGE, SET_SEARCH_VALUE,
} from '../../constants';
import massPagesRequest from '../../utils/massPagesRequest';
import limitMaxResul from '../../utils/limitMaxResult';
...
export const getContent = () => async (dispatch, getState) => {
  ...
  if (str.trim() && filter === '') {
    ...
    dispatch({ type: ADD_ITEMS, payload: limitMaxResul(result) });
  }
  if (filter === TRENDING) {
    dispatch({ type: SET_LOADING, payload: true });
